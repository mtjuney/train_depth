...
if __name__ == "__main__":
    ...
    for net in nets:
        for ex in range(n_ex):
            print("NeuralNet: Net{}".format(net))

            LOGPATH = LOGPATH_orig + "ex_{}/Net{}/".format(ex, net)
            os.makedirs(LOGPATH)

            try:
                if net == 2:
                    model = neuralnet.Net2()
                elif net == 3:
                    model = neuralnet.Net3()
                elif net == 4:
                    model = neuralnet.Net4()
                elif net == 6:
                    model = neuralnet.Net6()
                elif net == 8:
                    model = neuralnet.Net8()
                elif net == 10:
                    model = neuralnet.Net10()
                elif net == 12:
                    model = neuralnet.Net12()
