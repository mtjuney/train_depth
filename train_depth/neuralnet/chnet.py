...
class ChNet(chainer.FunctionSet):
    ...
    def __init__(self):
        super(ChNet, self).__init__(
            ...
            conv3b = F.Convolution2D(64, 32, 3, pad = 1),
            conv4b = F.Convolution2D(32, 1, 3, pad = 1),
        )
    ...
    def forward_super(self, x, train=True):
        ...
        h1 = F.average_pooling_2d(F.relu(self.conv3b(h)), self.p_w, stride=self.p_w)
        h = F.relu(self.conv3(h))
        h = F.relu(self.conv4(h))
        h = F.relu(self.conv5(h))

        h1 = F.average_pooling_2d(F.relu(self.conv4b(h1)), self.p_w, stride=self.p_w)
