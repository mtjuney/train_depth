...
LOGDIR = 'log/' + datetime.datetime.today().strftime('%Y-%m-%d_%H-%M-%S')
os.mkdir(LOGDIR)
LOGDIR += '/'

config_write = {'N_EPOCH':N_EPOCH, 'BATCHSIZE':BATCHSIZE, 'WIDTH_IMG':WIDTH_IMG, 'HEIGHT_IMG':HEIGHT_IMG, 'GPU':args.gpu}
f = open(LOGDIR + 'config.yml', 'w')
...
def add_record(row_array, record_type):
    if record_type == 'time':
        filename = LOGDIR + 'record_time.csv'
    else:
        filename = LOGDIR + 'record_loss.csv'
...
def read_image(filename, train_data=True):
    ...
    image_y = np.asarray(Image.open(y_path + filename).resize((WIDTH_IMG, HEIGHT_IMG))).reshape((NUM_PIXEL_I))
...
def feed_data():
    ...
    y_batch = np.ndarray((BATCHSIZE, NUM_PIXEL_I), dtype=np.float32)
    val_x_batch = np.ndarray((VAL_BATCHSIZE, 3, HEIGHT_IMG, WIDTH_IMG), dtype=np.float32)
    val_y_batch = np.ndarray((VAL_BATCHSIZE, NUM_PIXEL_I), dtype=np.float32)
