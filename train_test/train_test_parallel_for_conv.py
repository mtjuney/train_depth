...
def read_image(filename, train_data=True):
    ...
    image_y = np.asarray(Image.open(y_path + filename).resize((WIDTH_IMG, HEIGHT_IMG))).reshape((WIDTH_IMG, HEIGHT_IMG, 1)).transpose(2, 1, 0)
...
# Save final model
pickle.dump(model, open(LOGDIR + 'model', 'wb'), -1)
